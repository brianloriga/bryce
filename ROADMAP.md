...
- [x] 7.31 Read-aloud TTS ? OpenAI `nova` voice; reads question + A/B/C/D options; MP3s cached in Supabase Storage; `expo-av` playback in QuizScreen
...
- [x] 7.35 Reading Passage ? GPT extracts stories/articles from scanned pages; stored with unit; "?? Read Along" bar in QuizScreen opens a scrollable bottom-sheet modal
- [x] 7.36 Multiple Visual Aid slots ? 1/2/3 slots based on question count (5-9?1, 15?2, 20?3); parent picks questions-per-image (1/2/3); edge function handles per-image GPT instructions and `visual_urls` array
- [x] 7.37 Image resize pipeline ? `expo-image-manipulator` resizes all captures to 1024px/JPEG-70 before upload; 10 pages now fits well within 6MB Edge Function limit
- [x] 7.38 Scan UX ? question count picker appears after first photo (not before); Cancel button on generating screen; Cancel & Start Over below generate button; visual aid uses native crop editor (`allowsEditing: true`) instead of a Modal to fix iOS camera hang
...
## Phase 8 ? Boss Battles, Mini-Games & � La Carte Purchases

**Goal:** Reward kids who finish a quiz with an unlockable boss battle or mini-game. Parents choose which games attach to each unit. Individual games are purchasable � la carte ? a second monetization track alongside the subscription.
...
| **7.35-7.38 Scan & Quiz** | Reading passage, multi-visual-aid slots, image resize pipeline, scan UX (cancel, reorder, crop fix) | ? Done |
| 6 | App Store Launch (free / no paywall) | Pending |
| 8 | Boss battles, mini-games & � la carte purchases | Pending |
